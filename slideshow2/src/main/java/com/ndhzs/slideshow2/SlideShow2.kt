...
import com.ndhzs.slideshow.viewpager2.transformer.MultipleTransformer
...
import com.ndhzs.slideshow2.viewpager.Adapter
...
class SlideShow2 : CardView {
    ...
    fun <VH: RecyclerView.ViewHolder> setAdapter(adapter: RecyclerView.Adapter<VH>) {
        mViewPager.adapter = Adapter(adapter, mAttrs)
        mIndicators.forEach {
            setIndicators(it)
        }
    }
    ...
    fun getInnerAdapter(): Adapter<*>? {
        return mViewPager.adapter as Adapter<*>?
    }
    ...
    private fun getVp() = mViewPager // 为了逃避编译器检查

    private val mIndicators = ArrayList<AbstractIndicatorsView>(2)
    private val mPageChangeCallback by lazyUnlock { PageChangeCallback(getVp()) }
    private val mPageTransformers by lazyUnlock {
        MultipleTransformer().apply {
            getVp().setPageTransformer(this)
        }
    }
    ...
    private fun setPageInterval() {
        if (mViewPager.adapter is Adapter<*>) {
