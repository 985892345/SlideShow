...
class PageChangeCallback(
    ...
    private val innerAdapter: Adapter<*>
        get() = viewPager2.adapter as Adapter<*>
    ...
    private fun pageScrolledCallback(position: Int, positionOffset: Float, positionOffsetPixels: Int) {
        if (isCyclical) {
            if (mPositionFloat % outerAdapter.itemCount > outerAdapter.itemCount - 1) {// 当划出边界时
                val pf = (1 - positionOffset) * (outerAdapter.itemCount - 1)// 这里可以表示从右边界到左边界(或相反)经过的值
                mCallbacks.forEach {
    ...
    companion object {
        ...
        fun getOuterPos(innerAdapter: Adapter<*>, innerPosition: Int): Int {
         ...
         * 得到该回到的居中的位置
         */
        fun getCenterPos(currentItem: Int, innerAdapter: Adapter<*>): Int {
