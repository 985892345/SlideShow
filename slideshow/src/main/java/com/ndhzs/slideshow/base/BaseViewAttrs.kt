package com.ndhzs.slideshow.base
...
internal interface BaseViewAttrs {
  ...
  companion object {
    fun <T> newAttrs(
      ...
      val ty =
        view.context.obtainStyledAttributes(attrs, styleableId, defStyleAttr, defStyleRes)
  ...
  class Typedef(val ty: TypedArray, private val context: Context) {
    ...
    internal inline fun <reified E : RuntimeException> Int.intOrThrow(
      attrsName: String,
    ): Int = this.intOrThrow<E>(ty, attrsName)
    
    internal inline fun <reified E : RuntimeException> Int.stringOrThrow(
      attrsName: String,
    ): String = this.stringOrThrow<E>(ty, attrsName)
  }
}
